...
The following button will create the webhook with the correct scope
...
```{r,echo = FALSE}
details::details("`slackr::teams` is an `R6` object that allows the user to manage multiple Slack teams during a session. The object queries the [slackr-app](https://github.com/yonicd/slackr-app) database to set the environment variables needed by slackr. ",summary = '`teams`',lang = NULL,open = TRUE)
```

```{r,echo = FALSE}
details::details("The `slackr_setup()` function will try to read setup values from a `~/.slackr` (you can change the default) configuration file, which may be easier and more secure than passing them in manually (plus, will allow you to have multiple slackr configs for multiple Slack.com teams). The file is in Debian Control File (DCF) format since it really doesn't need to be JSON and R has a handy `read.dcf()` function since that's what `DESCRIPTION` files are coded in. 
...
You can also change the default emoji icon (from the one you setup at integration creation time) with `icon_emoji`.",summary = '`slackr_setup`',lang = NULL)
```
